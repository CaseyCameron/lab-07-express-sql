...
app.post('/api/monsters', async (req, res) => {
  try {
    ...
    `, [monster.name, monster.type, monster.hp, monster.ac, monster.cr, monster.isLegendary, monster.img_url, 1]); //monster.userId
...
app.get('/api/users/:id/monsters', async (req, res) => {
  // use SQL query to get data...
  try {
    const data = await client.query(`
      SELECT  *
